...
tn = as.double(tmp_var$TN)
fn = as.double(tmp_var$FN)
fp = as.double(tmp_var$FP)
tp = as.double(tmp_var$TP)
...
cat("Classification Error Rate is ", cer)
```
...
```{r}
f1score = 2 * prec * sen /(prec + sen)
cat("F1 Score is", f1score)
```
...
```{r}
svmROC <- roc(df$class ~ df$scored.class,plot=TRUE,print.auc=TRUE,col="blue",lwd = 4,print.auc.y=0.4,legacy.axes=TRUE,add = TRUE)
```
...
```{r}
plot.roc(df$scored.class, df$class,# data
