...
logRegModel1 <- glm(formula = TARGET_FLAG ~ . - TARGET_AMT, data=train, family = "binomial" (link="logit"))
...
```{r}
#(logistic regression)
logRegModel2 <- glm(formula = TARGET_FLAG ~ CLM_FREQ +
                      OLDCLAIM +
                      MVR_PTS +
                      HOME_VAL + 
                      URBANICITY +
...
For our third model, we only include the predictor variables that have `theoretical effect on probability of collition`, which was provided as part of the definition of the variables.
...
For our third model, we only include the predictor variables that have `theoretical effect on probability of collition`, which was provided as part of the definition of the variables. Additionally, we remove the variables that were deemed as "urban legends", such as `RED_CAR` and `SEX`.


```{r}
#(logistic regression)
logRegModel4 <- glm(formula = TARGET_FLAG ~ AGE +
                    CAR_USE +
                    CLM_FREQ +
                    EDUCATION +
                    HOME_VAL +
...
```{r}
#(logistic regression)
logRegModel5 <- glm(formula = TARGET_FLAG ~ AGE +
                    CAR_USE +
                    CLM_FREQ +
                    HOME_VAL +
                    INCOME +
